...
from app.application.dtos.inventory_dtos import (
    FindLowStockItemsCommand,
    FindOutOfStockItemsCommand,
)
...
@router.get(
    "/out-of-stock",
    response_model=PaginatedResponse[BaseBookResponse],
    operation_id="findOutOfStockItems",
)
async def find_out_of_stock_items(
    _admin_user: AdminUser,
    book_repo: BookRepo,
    db_session: DbSession,
    query: Annotated[ListQueryRequest, Depends()],
) -> PaginatedResponse[BaseBookResponse]:
    cmd = FindOutOfStockItemsCommand(
        query_params=QueryParams(
            page=PageParams(page=query.page, page_size=query.page_size),
            sorts=parse_sort(query.sort),
        ),
    )
    use_case = FindOutOfStockItemsUseCase(db_session=db_session, book_repo=book_repo)
    result = await use_case.execute(cmd)
    return PaginatedResponse(
        items=[build_base_book_response(b, BaseBookResponse) for b in result.items],
        pagination=PaginationMeta.from_result(result),
