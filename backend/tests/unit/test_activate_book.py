...
from app.application.use_cases.book.activate_book import ActivateBookUseCase
from app.core.query_params import PaginatedResult, QueryParams
from app.domain.entities import BookEntity
from app.domain.exceptions.book import BookAlreadyActivatedError, BookNotFoundError
...
) -> tuple[ActivateBookUseCase, AsyncMock]:
    db_session = AsyncMock()
    use_case = ActivateBookUseCase(
...
@pytest.mark.unit
class TestActivateBookUseCase:
    async def test_activates_deactivated_book(self) -> None:
        ...
        cmd = ActivateBookCommand(book_id="book-1")

        result = await use_case.execute(cmd)

        assert isinstance(result, ActivateBookResult)
    ...
    async def test_raises_book_not_found_when_missing(self) -> None:
        ...
        cmd = ActivateBookCommand(book_id="nonexistent")

        with pytest.raises(BookNotFoundError):
            await use_case.execute(cmd)

    async def test_raises_book_already_activated(self) -> None:
        book = _make_book("book-1", deactivated_at=None)  # already active
        book_repo = FakeBookRepository(book)
        use_case, _ = _build_use_case(book_repo)
        cmd = ActivateBookCommand(book_id="book-1")

        with pytest.raises(BookAlreadyActivatedError):
            await use_case.execute(cmd)
    ...
    async def test_soft_deleted_book_treated_as_not_found(self) -> None:
        ...
        use_case, _ = _build_use_case(book_repo)
        cmd = ActivateBookCommand(book_id="deleted-book")
