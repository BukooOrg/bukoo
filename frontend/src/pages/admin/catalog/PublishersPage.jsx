...
export default function PublishersPage() {
  ...
  const handleCreate = async (e) => {
    ...
    setError('');
    try {
      await publisherApi.createPublisher({
        createPublisherRequest: {
          name: name.trim(),
          website: website.trim() || undefined,
        },
      });
      toast.success(`Publisher "${name}" created`);
      setShowCreate(false);
      setName('');
      setWebsite('');
      fetchPublishers();
  ...
  const handleDelete = async () => {
    ...
    try {
      await publisherApi.softDeletePublisher({ publisherId: deleteTarget.id });
  ...
  return (
    <div className='space-y-8'>
      <div className='text-center pt-8'>
        ...
        <p className='text-primary/40 font-bold italic text-sm'>{publishers.length} publishers</p>
      </div>
              ...
              {publishers.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className='py-16 text-center'>
                    ...
                    <p className='font-serif text-lg italic text-primary/30'>No publishers yet</p>
                  </TableCell>
                </TableRow>
              ) : (
                publishers.map((pub) => (
                  <TableRow key={pub.id}>
                    <TableCell className='font-sans font-bold text-primary'>
                      <Link to={`/admin/publishers/${pub.id}`} className='hover:text-primary/70'>
                        {pub.name}
                      </Link>
                    </TableCell>
