import { FolderTree, Plus, Pencil, Trash2, Loader2, AlertCircle } from 'lucide-react';
...
export default function CategoriesPage() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      setCollections(collRes.data?.items || []);
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      await categoryApi.softDeleteCategory({ categoryId: deleteTarget.id });
  ...
  return (
    <div className='space-y-8'>
      <div className='text-center pt-8'>
        ...
        <p className='text-primary/40 font-bold italic text-sm'>{categories.length} categories</p>
      </div>
              ...
              {categories.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className='py-16 text-center'>
                    ...
                    <p className='font-serif text-lg italic text-primary/30'>No categories yet</p>
                  </TableCell>
                </TableRow>
              ) : (
                categories.map((cat) => (
                  <TableRow key={cat.id}>
                    <TableCell className='font-sans font-bold text-primary'>
                      <Link to={`/admin/categories/${cat.id}`} className='hover:text-primary/70'>
                        {cat.name}
                      </Link>
                    </TableCell>
                    <TableCell className='text-sm text-primary/40'>{cat.urlSlug || '—'}</TableCell>
