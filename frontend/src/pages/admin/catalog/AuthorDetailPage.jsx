...
import { authorApi, bookApi } from '@/lib/apiClient';
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [authorRes, booksRes] = await Promise.all([
        authorApi.viewAuthorDetail({ authorId }),
        bookApi.findBooks({ authorId, pageSize: 50, sort: 'title:asc' }),
      ]);
      ...
      <div className='p-6 bg-white/40 border border-primary/5 rounded-2xl'>
        ...
        {(() => {
          ...
          if (relatedBooks.length === 0) {
            return (
              <p className='text-sm text-primary/30 italic'>No books associated with this author</p>
            );
          }
