...
export function BookViewer3D({ frontCoverUrl, backCoverUrl }) {
    ...
    if (bookReady && !hasInitialAnimationPlayed) {
      ...
      const animate = () => {
        ...
        setParams(prev => ({
          ...prev,
          rotation: [
            initialParams.rotation[0] + (defaultParams.rotation[0] - initialParams.rotation[0]) * easedProgress,
            initialParams.rotation[1] + (defaultParams.rotation[1] - initialParams.rotation[1]) * easedProgress,
            initialParams.rotation[2] + (defaultParams.rotation[2] - initialParams.rotation[2]) * easedProgress,
          ],
          cameraFov: initialParams.cameraFov + (defaultParams.cameraFov - initialParams.cameraFov) * easedProgress
        }))
  ...
  return (
    <div className="w-full h-full min-h-[400px] lg:min-h-[600px] bg-transparent transition-opacity duration-1000" style={{ opacity: bookReady ? 1 : 0 }}>
      <BookModel
        params={params}
        meshRef={meshRef}
